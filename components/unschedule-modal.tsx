...
interface UnscheduleModalProps {
  ...
  scheduledLocations: TripLocation[]; // already in trip days — hide these
  unscheduledLocations: TripLocation[];
  onSave: (locations: TripLocation[]) => void;
}

export function UnscheduleModal({ open, onOpenChange, scheduledLocations, unscheduledLocations, onSave }: UnscheduleModalProps) {
  const [searchQuery, setSearchQuery] = useState('');
  // local draft of unscheduled picks; initialised from current state each open
  const [selected, setSelected] = useState<TripLocation[]>([]);

  // Reset local state when opening
  const handleOpenChange = (val: boolean) => {
    if (val) setSelected(unscheduledLocations);
    else setSelected([]);
    onOpenChange(val);
  };

  const scheduledNames = new Set(scheduledLocations.map(l => l.name));
  const selectedNames = new Set(selected.map(l => l.name));

  const visible = allLocations.filter(loc => {
    if (scheduledNames.has(loc.title)) return false; // hide scheduled ones
  ...
  const toggle = (loc: typeof allLocations[0]) => {
    if (selectedNames.has(loc.title)) {
      setSelected(selected.filter(l => l.name !== loc.title));
    } else {
      setSelected([...selected, {
        id: `unscheduled-${loc.title}`,
        name: loc.title,
      ...
      >
        <div className="flex flex-col" style={{ height: '80vh', maxHeight: '80vh' }}>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
            <h2 className="text-lg font-semibold text-gray-900">Add Unscheduled Location</h2>
            <button
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-4">
            {visible.length === 0 ? (
              ...
              <div className="grid grid-cols-3 gap-3">
                {visible.map(loc => {
                  const isSelected = selectedNames.has(loc.title);
                  const globalIdx = selected.findIndex(l => l.name === loc.title);
                  return (
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 flex-shrink-0">
            ...
            <button
              onClick={() => { onSave(selected); handleOpenChange(false); }}
